...
function addAccessors($scope) {
    ...
    function addBackgroundImage(imageName) {
        var src = '../assets/' + imageName;
    ...
    function addImageX(imageName) {
        ...
        fabric.Image.fromURL('../assets/' + imageName, function(image) {
            image.set({
                ...
                .scale(0.8)
                .setCoords();
            canvas.add(image).sendToBack(image);
        });
    };

    $scope.addImagex = function(name) {
        addImageX(name);
    };

    $scope.addBackgroundImageX = function(name) {
        addBackgroundImage(name);
    };
    ...
    $scope.rasterizePNG = function() {
        canvas.renderAll();
        window.open(
            'data:image/png+xml;utf8,' +
    ...
    $scope.grayscale = function(count) {
        ...
        if (count > 0) {
            ...
            obj.filters[0] = new filter.Grayscale();
        }
        obj.applyFilters();
        canvas.renderAll();
    ...
    $scope.sepia = function(count) {
        ...
        if (count > 0) {
            ...
            obj.filters[3] = new filter.Sepia();
        }
        obj.applyFilters();
        canvas.renderAll();
    };

    $scope.brownie = function(count) {
        var filter = fabric.Image.filters;
        var obj = canvas.getActiveObject();
        if (!obj) return;
        ...
        if (count > 0) {
            ...
            obj.filters[4] = new filter.Brownie();
        }
        obj.applyFilters();
        canvas.renderAll();
    };

    $scope.border = function(count) {
        var filter = fabric.Image.filters;
        var obj = canvas.getActiveObject();
        if (!obj) return;
        ...
        if (count > 0) {
            ...
            obj.set('radius', 75);
            obj.set('backgroundColor', '#E0E0E0');
            obj.setShadow({
                color: 'rgba(0,0,0,0.3)',
                blur: 5,
                offsetX: 2,
